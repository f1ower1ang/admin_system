<template>
  <div class="ioc-wrapper">
    ...
    <el-button plain style="position:fixed; top: 150px; left: 220px">
      <a :href="url" style="color: #000" target="_blank">导出到Excel</a>
    </el-button>
...
<script>
import { getIocDir, getIocDomain, getIocIp, getIocSample, getIocEmail, getIocUrl } from '../../api/detail'
...
export default {
  ...
  data () {
    return {
      ...
      dom: '暂无结果',
      flag: false,
      url: `/trees/excelExport?name=${this.$route.query.name}`
    }
  },
  mounted () {
  ...
  created () {
    getIocDir(this.$route.query.name).then((res) => {
      this.dir = res.data
